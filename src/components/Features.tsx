  ...
  const features = [
    {
      ...
      description:
        "Trouvez les plats disponibles dans un rayon précis autour de votre position actuelle.",
    },
    {
      icon: <Calendar className="w-8 h-8 text-primary" />,
      title: "Réservation en ligne",
      description:
        "Pour les restaurants partenaires, réservez votre table directement depuis l'application.",
    },
    {
      icon: <Star className="w-8 h-8 text-primary" />,
      title: "Favoris & Avis",
      description:
        "Sauvegardez vos restaurants préférés et notez les plats que vous avez dégustés.",
    },
    {
      icon: <Bell className="w-8 h-8 text-primary" />,
      title: "Notifications",
      description:
        "Soyez alerté dès que votre restaurant favori publie son menu du jour.",
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <div
              key={index}
              className="group p-6 bg-white rounded-2xl border border-gray-100 hover:border-primary/30 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1"
            >
